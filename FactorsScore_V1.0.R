...
if(!is.null(opt$h) || is.null(opt$input) || is.null(opt$time) || is.null(opt$outdir) || is.null(opt$prefix)){
        ...
        CNV： row names are samples, columns names are genes；
        RNA： (1) fpkm/tpm by gene：row names are samples, column names are genes;
              (2) fpkm/tpm by transcript：column names are samples, row names are genes.
        format of time file：the title of this file must be "sample    OS  re" separated by tab.", sep = "\n")
        q(status=1)
}
